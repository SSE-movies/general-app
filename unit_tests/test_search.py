...
def test_search_results(client, auth_headers):
    ...
    response = client.get("/results?categories=Action%20%26%20Adventure", headers=auth_headers)
...
def test_empty_search_results(client, auth_headers):
    ...
    response = client.get("/results?title=nonexistentmovie123456", headers=auth_headers)
