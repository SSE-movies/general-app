...
def test_duplicate_watchlist_add(auth_headers, test_movie):
    ...
    auth_headers.post("/add_to_watchlist", json={"showId": test_movie["showId"]})

    response = auth_headers.post("/add_to_watchlist", json={"showId": test_movie["showId"]})
...
def test_remove_from_watchlist(auth_headers, test_movie):
    ...
    response = auth_headers.post("/remove_from_watchlist", json={"showId": test_movie["showId"]})
    assert response.status_code == 200, f"Unexpected status code: {response.status_code}"
...
def test_mark_watched(auth_headers, test_movie):
    ...
    response = auth_headers.post("/mark_watched", json={"showId": test_movie["showId"]})
    assert response.status_code == 200, f"Unexpected status code: {response.status_code}"
...
def test_mark_unwatched(auth_headers, test_movie):
    ...
    response = auth_headers.post("/mark_unwatched", json={"showId": test_movie["showId"]})
    assert response.status_code == 200, f"Unexpected status code: {response.status_code}"
