...
def test_remove_from_watchlist(auth_headers, test_movie):
    ...
    # Then remove it
    response = auth_headers.post(
        "/remove_from_watchlist", data={"showId": test_movie["showId"]}
    )
...
def test_mark_watched(auth_headers, test_movie):
    ...
    # Mark as watched
    response = auth_headers.post(
        "/mark_watched", data={"showId": test_movie["showId"]}
    )
...
def test_mark_unwatched(auth_headers, test_movie):
    ...
    auth_headers.post("/mark_watched", data={"showId": test_movie["showId"]})

    # Then mark as unwatched
    response = auth_headers.post(
        "/mark_unwatched", data={"showId": test_movie["showId"]}
    )
    assert response.status_code == 302
