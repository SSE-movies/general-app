...
def test_search_page_access(auth_client):  # Changed from (client, auth_headers)
    """Ensure logged-in users can access the search page."""
    response = auth_client.get("/search")  # No need for headers with session auth
...
def test_search_results(auth_client):  # Changed from (client, auth_headers)
    ...
    response = auth_client.get(
        "/results?categories=Action%20%26%20Adventure"
    )

    assert response.status_code == 200, f"Unexpected status code: {response.status_code}"
    assert (b"Action" in response.data or b"Adventure" in response.data), \
    ...
    assert response.status_code == 200, f"Unexpected status code: {response.status_code}"
    assert (b"test" in response.data or b"Movie" in response.data), \
           f"Expected title missing in results: {response.data}"

def test_empty_search_results(auth_client):  # Changed from (client, auth_headers)
    """Ensure a search with no results displays the correct message."""
    response = auth_client.get(
        "/results?title=nonexistentmovie123456"
    )

    assert response.status_code == 200, f"Unexpected status code: {response.status_code}"
    assert (b"No movies found" in response.data or b"No results" in response.data), \
